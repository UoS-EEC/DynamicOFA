...
import pickle
import psutil
def evaluate_ofa_subnet(ofa_net, path, net_config, data_loader, batch_size,bn_list,usage, maximum,device='cuda:0'):
    ...
    #ed = time.time()
    #bn_list = []
    #print('set subnet latency',ed-st)
    subnet = ofa_net.get_active_subnet().to(device)
    #ed = time.time()
    #print('get subnet latency',ed-st)
    #bn_list = calib_bn(subnet, path, net_config['r'][0],32)
    bn_mean = bn_list[0]
    bn_var = bn_list[1]
    
    #bn_list = calib_bn(subnet,data_loader,bn_mean,bn_var)

    #print('calib_bn latency',ed-st)
    calib_bn(subnet,data_loader,bn_mean,bn_var)
    ed = time.time()
    init_latency = ed-st
    #print('start validatiuon')
    
    top1,latency = validate(subnet, path, net_config['r'][0], data_loader,usage,maximum,batch_size,device='cuda:0')


    return top1,latency,init_latency
'''
def calib_bn(net, path, image_size, batch_size, num_images=2000):
    ...
    bn_list = set_running_statistics(net, data_loader)
    return bn_list
...
def validate(net, path, image_size, data_loader,usage,maximum,batch_size=100, device='cuda:0'):
    if 'cuda' in device:
    ...
    with torch.no_grad():
        ...
        for i, (images, labels) in enumerate(data_loader):
            ...
            #monitor = torch.cuda.memory_allocated(device='cuda:0')
            #monitor = torch.cuda.memory_summary(device='cuda:0')
            #memory = psutil.Process(os.getpid()).memory_info().rss
                       

            #usage_new = psutil.virtual_memory().percent
            #f = open('usage_new.txt','a')
            #f.write(str(usage_new)+'\n')
            #f.close()
            #print('Current memory allocated percentage',usage)
            #if monitor > maximum*0.6:
                #return 0,0
            #if monitor > maximum * (100-usage)/100:
                #return 0,0
                #raise RuntimeError('Memory out')
            #print('Current memory allocated',monitor)
            #torch.cuda.synchronize()
            ed = time.time()
            ...
            if i > 20:
                if (ed-st)*1000 < 90:
                    record.append(ed-st)
                    if (sum(record[-20:])/20)*1000 > maximum:
                        return 0,0
            f = open('judge.txt')
            sign = f.readline()
            f.close
            ...
            if maximum != 1000:
                if len(latency) > 20 and sign=='True':
                    return 1,1
