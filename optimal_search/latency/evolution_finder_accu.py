...
class EvolutionFinder:
	...
	def random_sample(self):
		...
		while True:
			...
			#print(sample)
			acc = self.accuracy_predictor.predict_accuracy([sample])
			if efficiency <= constraint and acc >= accuracy_constraint-accuracy_r:
	...
	def mutate_sample(self, sample):
		...
		while True:
			...
			acc = self.accuracy_predictor.predict_accuracy([new_sample])
			if efficiency <= constraint and acc >= accuracy_constraint:
	...
	def run_evolution_search(self, verbose=False):
		...
		for _ in range(population_size):
			sample, efficiency,accuracy = self.random_sample()
			child_pool.append(sample)
			efficiency_pool.append(efficiency)
			accuracy_pool.append(accuracy)

		#accs = self.accuracy_predictor.predict_accuracy(child_pool)
		for i in range(mutation_numbers):
			population.append((accuracy_pool[i].item(), child_pool[i], efficiency_pool[i]))

		if verbose:
			print('Start Evolution...')
		# After the population is seeded, proceed with evolving the population.
		for iter in tqdm(range(max_time_budget), desc='Searching with %s constraint (%s) accuracy (%s)' % (self.constraint_type, self.efficiency_constraint,self.accuracy_constraint)):
			...
			for i in range(mutation_numbers):
				...
				# Mutate
				new_sample, efficiency,accuracy = self.mutate_sample(par_sample)
			...
			for i in range(population_size - mutation_numbers):
				...
				# Crossover
				new_sample, efficiency,accuracy = self.crossover_sample(par_sample1, par_sample2)
				child_pool.append(new_sample)
				efficiency_pool.append(efficiency)
				accuracy_pool.append(accuracy)
