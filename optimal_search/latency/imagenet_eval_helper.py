...
def evaluate_ofa_subnet(ofa_net, path, net_config, data_loader, batch_size, device='cuda:0'):
    ...
    bn_list = calib_bn(subnet, path, net_config['r'][0],batch_size)
    #bn_mean = bn_list[0]
    #bn_var = bn_list[1]
    ...
    return top1,latency,init_latency,bn_list
...
def calib_bn(net,data_loader,bn_mean,bn_var):
    print('Creating dataloader for resetting BN running statistics...')
    bn_list = set_running_statistics(net, data_loader,bn_mean,bn_var)
'''
